...
export const LANG_IDS_WITH_DESTINATION = [
  ['Assembly', 'asm', 'sh'],
  ['Batchfile', 'bat', 'sh'],
  ['C', 'c', undefined],
  ['C#', 'cs', undefined],
...
export const LANGID_AND_EXTENSIONS = LANG_IDS_WITH_DESTINATION.reduce(
  (acc, [langId, extension]) => {
    if (!langId || !extension) {
      return acc
    }

    return acc.set(langId.toLocaleLowerCase(), extension)
  },
  new Map<string, string>(),
)

export const LANGUAGES = new Map(
  LANG_IDS_WITH_DESTINATION.map(([, source, dest]) => [source, dest]),
)
